...
class Command(BaseCommand):
    ...
    def print_help(self, *args):
        message = '''
        账户管理命令用法：
            user add    创建账户，例如：user add -u admin -p admin -n 管理员 -s
    ...
    def handle(self, *args, **options):
        ...
        if action == 'add':
            if not all((options['u'], options['p'], options['n'])):
                ...
                self.print_help()
            elif User.objects.filter(username=options['u'], deleted_at__isnull=True).exists():
            ...
            if not options['u']:
                self.echo_error('缺少参数')
                self.print_help()
            user = User.objects.filter(username=options['u'], deleted_at__isnull=True).first()
            if user:
                user.is_active = True
                user.save()
